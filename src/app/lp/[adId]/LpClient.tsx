...
function LpDemo({ adId, onFirstMessage }: { adId: string; onFirstMessage: () => void }) {
  ...
  return (
    <div className="mx-auto w-full max-w-sm">
      {/* Phone frame */}
      <div className="overflow-hidden rounded-[2rem] border border-white/[0.10] bg-[#0f0f0f] shadow-2xl">
        {/* Status bar */}
          ...
          <div className="flex flex-wrap gap-1.5 px-4 pb-2">
            {QUICK_REPLIES.map((r) => (
              <button
                ...
                type="button"
                onClick={() => send(r)}
                className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/60 transition-colors hover:border-white/25 hover:text-white/90"
              >
                {r}
              </button>
            ))}
        ...
        <div className="flex items-center gap-2 border-t border-white/[0.06] px-4 py-3">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-white disabled:opacity-30"
          >
...
function CtaButton({ adId, label = "Build my agent — Free" }: { adId: string; label?: string }) {
  return (
    <a
      ...
      className="block w-full rounded-xl bg-white py-4 text-center text-base font-semibold text-[#0a0a0a] transition-opacity hover:opacity-90 active:opacity-80"
    >
      {label}
    </a>
  );
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <header className="flex items-center justify-between px-5 py-4">
        ...
        <a
          ...
          className="rounded-lg bg-white px-4 py-2 text-xs font-semibold text-[#0a0a0a] transition-opacity hover:opacity-90"
        >
          Get started free
        </a>
      </header>
      ...
      <section className="px-5 pb-8 pt-6 text-center">
        <h1 className="mb-4 text-[2rem] font-bold leading-[1.15] tracking-tight text-white sm:text-5xl">
          {h1}
        </h1>
        <p className="mx-auto mb-8 max-w-md text-base leading-relaxed text-white/55 sm:text-lg">
          {subtitle}
        </p>
      ...
      <section className="px-5 pb-10">
        <div className="mx-auto max-w-sm overflow-hidden rounded-2xl border border-white/[0.08] bg-black shadow-2xl">
          <video
            ref={videoRef}
            src={video}
            autoPlay
